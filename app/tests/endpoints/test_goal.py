...
def test_create_goal_student_not_in_group(
...
def test_create_goal_teacher_not_in_group(
        client, session, normal_user_token_headers
):
    user = get_user_from_token_headers(client, normal_user_token_headers)
...
def test_create_goal_user_is_not_teacher(
        client, session, normal_user_token_headers
):
    user = get_user_from_token_headers(client, normal_user_token_headers)
...
def test_create_goal_non_exist_student(
        client, session, normal_user_token_headers
):
    user = get_user_from_token_headers(client, normal_user_token_headers)
    teacher = update_user(session, user, {'role': Role.teacher})
...
def test_create_goal_non_exist_standard(
        client, session, normal_user_token_headers
):
    user = get_user_from_token_headers(client, normal_user_token_headers)
    teacher = update_user(session, user, {'role': Role.teacher})
...
def test_create_goal_non_exist_group(
    client, session, normal_user_token_headers
):
    user = get_user_from_token_headers(client, normal_user_token_headers)
    teacher = update_user(session, user, {'role': Role.teacher})
...
def test_create_goal_accuracy_over_100(
    client, session, normal_user_token_headers
):
    user = get_user_from_token_headers(client, normal_user_token_headers)
    teacher = update_user(session, user, {'role': Role.teacher})
...
def test_create_goal_accuracy_is_0(
    client, session, normal_user_token_headers
):
    user = get_user_from_token_headers(client, normal_user_token_headers)
    teacher = update_user(session, user, {'role': Role.teacher})
...
def test_create_goal_accuracy_negative(
    client, session, normal_user_token_headers
):
    user = get_user_from_token_headers(client, normal_user_token_headers)
    teacher = update_user(session, user, {'role': Role.teacher})
...
def test_create_goal_not_future_end_date(
    client, session, normal_user_token_headers
):
    user = get_user_from_token_headers(client, normal_user_token_headers)
    teacher = update_user(session, user, {'role': Role.teacher})
...
def test_get_goal_student(client, session, normal_user_token_headers):
    ...
    assert data['standard'] == StandardRead.from_orm(standard).dict()
    assert data['resources'] == [
        ResourceReadWithCards.from_orm(resource).dict()]
...
def test_get_goal_as_student_not_on_goal(
        client, session, normal_user_token_headers
):
    teacher = create_random_user(session, role=Role.teacher)
...
def test_get_goal_as_teacher_not_on_goal(
        client, session, normal_user_token_headers
):
    user = get_user_from_token_headers(client, normal_user_token_headers)
    teacher = update_user(session, user, {'role': Role.teacher})
...
def test_add_resource_not_goal_teacher(
        client, session, normal_user_token_headers
):
    ...
    standard = create_random_standards(session, topic, 1)
    goal = create_random_goals(session, other_teacher, student, group, standard, n=1)
    resource = create_random_resources(
        session, other_teacher, all_private=True)
    create_random_cards(session, resource, 5)
    response = client.post(
        f'/goal/resource-link/',
        json={'goal_id': goal.id, 'resource_id': resource.id},
...
def test_add_private_resource_non_creator(
        client, session, normal_user_token_headers
):
    other_user = create_random_user(session)
    user = get_user_from_token_headers(client, normal_user_token_headers)
...
def test_add_public_resource_non_creator(
        client, session, normal_user_token_headers
):
    other_user = create_random_user(session)
    user = get_user_from_token_headers(client, normal_user_token_headers)
...
def test_add_non_exist_resource(
        client, session, normal_user_token_headers
):
    user = get_user_from_token_headers(client, normal_user_token_headers)
    teacher = update_user(session, user, {'role': Role.teacher})
...
def test_add_resource_non_exist_goal(
        client, session, normal_user_token_headers
):
    user = get_user_from_token_headers(client, normal_user_token_headers)
    teacher = update_user(session, user, {'role': Role.teacher})
...
def test_add_multi_resources(client, session, normal_user_token_headers):
    ...
    data = response.json()
    assert data['resources'] == [ResourceReadWithCards.from_orm(r).dict()
                                 for r in resources]


def test_add_multi_resources_non_exist_goal(
        client, session, normal_user_token_headers
):
    user = get_user_from_token_headers(client, normal_user_token_headers)
    teacher = update_user(session, user, {'role': Role.teacher})
...
def test_add_multi_non_exist_all_resources(
        client, session, normal_user_token_headers
):
    user = get_user_from_token_headers(client, normal_user_token_headers)
    teacher = update_user(session, user, {'role': Role.teacher})
...
def test_add_multi_resources_some_non_exist(
    client, session, normal_user_token_headers
):
    user = get_user_from_token_headers(client, normal_user_token_headers)
    teacher = update_user(session, user, {'role': Role.teacher})
...
def test_add_multi_resources_creator_mixed(
        client, session, normal_user_token_headers
):
    ...
    data = response.json()
    assert response.status_code == 200
    exp_resources = [r for r in all_resources
                     if r.creator == teacher or not r.private]
    assert data['resources'] == [ResourceReadWithCards.from_orm(r).dict()
