...
def test_create_lap(client, session):
    goal = create_random_goals_with_resources(session, n_rsc_per=1, n_cards_per=3)
...
def test_create_laps_all_goal_resources(client, session):
    ...
    for resource in goal.resources:
        ...
        goal_resource = crud.goal_resource.get_link(
            session, goal_id=goal.id, resource_id=resource.id
        )
...
def test_get_lap_as_teacher(client, session):
    ...
    headers = authentication_token_from_email(client, session, goal.teacher.email)
...
def test_get_lap_as_student(client, session):
    ...
    lap = create_random_laps(session, goal, resource)
    create_random_attempts(session, lap)
    headers = authentication_token_from_email(client, session, goal.student.email)
    response = client.get(f"lap/{lap.id}", headers=headers)
    data = response.json()
    # pprint_dict(data)
    assert response.status_code == 200
...
def test_get_lap_non_member_teacher(client, session):
    ...
    headers = authentication_token_from_email(client, session, other_teacher.email)
    response = client.get(f"lap/{lap.id}", headers=headers)
    data = response.json()
    pprint_dict(data)
    assert response.status_code == 401
...
def test_get_lap_non_member_student(client, session):
    ...
    lap = create_random_laps(session, goal, resource)
    other_student = create_random_user(session, Role.student)
    headers = authentication_token_from_email(client, session, other_student.email)
    response = client.get(f"lap/{lap.id}", headers=headers)
    data = response.json()
    pprint_dict(data)
    assert response.status_code == 401
