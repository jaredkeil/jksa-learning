...
def test_create_resource_all_fields(normal_user_token_headers, client):
    response = client.post(
        '/resource/',
        headers=normal_user_token_headers,
        json={'name': 'A Fully flushed out resource',
              'private': 'false',
              'format': 'pdf'
              }
    )
...
def test_get_private_resource_as_creator(
...
def test_get_private_as_not_creator(
        client, session, normal_user_token_headers
):
    ...
    response = client.get(f'/resource/{resource_not_mine.id}',
                          headers=normal_user_token_headers)

    assert response.status_code == 401


def test_get_public_resource_as_creator(
        client, session, normal_user_token_headers
):
    test_user = get_user_from_token_headers(client, normal_user_token_headers)
    resource_in = ResourceCreateInternal(
    ...
    response = client.get(f'/resource/{resource_public.id}',
                          headers=normal_user_token_headers)
...
def test_get_public_resource_as_not_creator(
        client, session, normal_user_token_headers
):
    user_not_me = create_random_user(session)
    resource_in = ResourceCreateInternal(
...
def test_get_resources_standard(client, session, normal_user_token_headers):
    ...
    response = client.get(
        f'/resource/?standard_id={standard.id}',
        headers=normal_user_token_headers
    )
    data = response.json()  # list of dicts
    assert response.status_code == 200
...
def test_get_resources_standard_non_exist(
    client, session, normal_user_token_headers
):
...
def test_add_standard_link_not_creator(
        client, session, normal_user_token_headers):
    topic = create_topics(session, 1)
    standard = create_random_standards(session, topic, 1)
    user_not_me = create_random_user(session)
...
def test_add_standard_link_non_auth(
        client, session, normal_user_token_headers
):
    topic = create_topics(session, 1)
    standard = create_random_standards(session, topic, 1)
...
def test_resource_update_not_creator(
        client, session, normal_user_token_headers
):
