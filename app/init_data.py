...
from app.models import (
    UserCreate, UserRead, TopicCreate, Topic, Subject, StandardCreate,
    Standard,
...
def dummy_data(superuser: User):
    with Session(engine) as session:
        ...
        resources_cards = dummy_cards(session,
                                      resources)  # 3 cards per resource
...
def dummy_cards(session: Session, resources: list[Resource]
    ...
    for r in resources:
        card_in1 = CardCreate(question=f'{r.id}:Q 1', answer=f'{r.id}A 1',
                              resource_id=r.id)
        card_in2 = CardCreate(question=f'{r.id}:Q 2', answer=f'{r.id}A 2',
                              resource_id=r.id)
        card_in3 = CardCreate(question=f'{r.id}:Q 3', answer=f'{r.id}A 3',
                              resource_id=r.id)
