...
from app.tests.tools.mock_params import (
    random_int,
    random_email,
    random_lower_string,
    random_subject,
...
def create_random_user(session: Session, role: Role = None) -> User:
    ...
    """
    user_in = UserCreate(email=random_email(), password=random_password(), role=role)
...
def create_random_resource(session: Session, user: User) -> Resource:
    ...
    """
    resource_in = ResourceCreateInternal(name=random_lower_string(), creator_id=user.id)
...
def create_random_standards(
    session: Session, topic: Topic, n: int = 1
...
def create_random_cards(
    session: Session, resource: Resource, n: int = 1
) -> list[Card] | Card:
    cards = []
    for i in range(n):
        card_in = CardCreate(
...
def create_random_goals_with_resources(
    session, n: int = 1, n_rsc_per: int = 2, n_cards_per: int = 3
) -> list[Goal] | Goal:
    goals = []
    for _ in range(n):
...
def create_random_laps(
    session: Session, goal: Goal, resource: Resource, n: int = 1
) -> list[Lap] | Lap:
    """Create random laps"""
    laps = []
    for _ in range(n):
...
def create_random_attempts(session: Session, lap: Lap) -> list[Attempt]:
    ...
    for card in lap.resource.cards:
        attempt_in = AttemptCreateInternal(
            submission=random_lower_string(8),
            lap_id=lap.id,
            card_id=card.id,
