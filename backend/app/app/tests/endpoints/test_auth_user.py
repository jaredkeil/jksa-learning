...
from app.tests.tools.mock_user import get_user_from_token_headers, random_password
...
def test_create_user_minimal(client):
    response = client.post(
        "auth/signup",
        json={
            "email": "create_user_minimal@example.com",
...
def test_create_user_all_fields(client):
    response = client.post(
        "/auth/signup",
        json={
            "email": "create_user_all_fields@example.com",
            "password": random_password(),
            "first_name": "John",
            "last_name": "Smith",
...
def test_create_user_invalid_email(client):
    response = client.post(
        "/auth/signup", json={"email": "invalid.com", "password": random_password()}
    )
...
def test_create_user_missing_password(client):
    response = client.post("/auth/signup", json={"email": random_email()})
...
def test_create_user_invalid_password(client):
    response = client.post(
        "/auth/signup", json={"email": random_email(), "password": "no numbers"}
    )
    assert response.status_code == 422
    assert response.json()["detail"][0]["loc"] == ["body", "password"]
...
def test_create_user_duplicate(client, session):
    ...
    response = client.post(
        "/auth/signup", json={"email": user.email, "password": random_password()}
    )
...
def test_login(client, session):
    ...
    response = client.post(
        "/auth/login",
        data={"username": user.email, "password": user_in.password.get_secret_value()},
    )
    data = response.json()
...
def test_login_incorrect_password(client, session):
    ...
    response = client.post(
        ...
        data={
            "username": user.email,
            "password": user_in.password.get_secret_value()[::-1],
        },  # :p
    )
    data = response.json()
    assert response.status_code == 400
...
def test_get_all_users_as_non_superuser(client, session, normal_user_token_headers):
